...
def hash_function(data, subkeys):
    data = padding_data(data)
    blocks = [data[i:i + 8] for i in range(0, len(data), 8)]
    hash_value = 525252
    ...
    for block in blocks:
        block_int = int.from_bytes(block, 'big')
        hash_value ^= encrypt_block(block_int, subkeys)
    
    hash_value = encrypt_block(hash_value, subkeys)
...
data = b"Example data for hash-function"
hashed_value = hash_function(data, subkeys)

print(f"data: {data}")
print(f"hash: {hashed_value.hex()}")
